...
$( document ).ready(function() {
	...
	for (i=0; i<=(yArray.length-1); i++) {
		for(j=0; j<=(xArray.length-1); j++ ) {
			grid.push({"x": xArray[j], "y": yArray[i]})
		}
	}
	...
	for (i=0; i<=(grid.length-1); i++) {
		...
		xPos = grid[i]['x'],
		yPos = grid[i]['y'];
		bigStyle.push(xPos +'px ' + yPos + 'px 0 #ff0000');
	}
	...
	for (i=0; i<=(logo.length-1); i++) {
		...
		xPos = logo[i]['x'],
		yPos = logo[i]['y'];
		logoName.push(xPos +'px ' + yPos + 'px 0 #000000');
	}

	$('a.clickme').on('click',function(){
		$('.logojson').css({"box-shadow": logoName.concat(bigStyle)});
	});

	$('a.clickme2').on('click',function(){
		$('.logojson').css({"box-shadow": logoName});
	});
});
...
// 	$('.logojson').css({'box-shadow': bigStyle});
